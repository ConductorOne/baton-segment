...
The Twilio Segment connector supports [automatic account provisioning and deprovisioning](/docs/product/admin/account-provisioning). Provisioning an account sends a workspace invitation; deprovisioning removes the user from the workspace.
...
- Role assignments can be granted and revoked for users only. Groups can hold roles in Segment, and C1 syncs those assignments, but C1 cannot change them.
